 ...
 *
 */
public class Soluation_133502 {
	public static void main(String[] args) {

		int[] ingredient = new int[] { 2, 1, 1, 2, 3, 1, 2, 3, 1 };

		System.out.println(run(ingredient));
	}

	private static int run(int[] ingredient) {
		int result = 0;
		...
		for (int i = 0; i < ingredient.length; i++) {
			...
			if (stack.size() >= 4) {
				...
				// 2,1,1,2 => x
				// 2,1,1,2,3 => x
				// 2,1,1,2,3,1 => o (1,2,3,1)
				if (stack.get(stackSize - 1) == 1 && stack.get(stackSize - 2) == 3 && stack.get(stackSize - 3) == 2
