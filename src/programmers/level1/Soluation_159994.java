...
public class Soluation_159994 {
	public static void main(String[] args) {
		...
		run(cards1, cards2, goal);
	}

	private static String run(String[] cards1, String[] cards2, String[] goal) {
		int c1 = 0;
		int c2 = 0;

		for (int i = 0; i < goal.length; i++) {
			String target = goal[i];
			if (c1 < cards1.length && cards1[c1].equals(target)) {
				c1++;
			} else if (c2 < cards2.length && cards2[c2].equals(target)) {
				c2++;
			} else {
				return "No";
			}
